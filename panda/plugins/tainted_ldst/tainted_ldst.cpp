...
void log_ldst(Addr a, uint64_t size, bool is_load) {
    ...
    if (num_tainted > 0) {
//        cout << "log_ldst is_load=" << is_load << " a=" << a.val.la << " o=" << a.off << "\n";
...
void tainted_load(Addr a, uint64_t size) {
...
// a is addr of ptr. 
// srcdest is value of ptr for load/str -- not needed
void tainted_store(Addr a, uint64_t size) {
...
bool init_plugin(void *self) {
    if (!pandalog) {
        printf ("tainted_ldst needs a pandalog\n");
        return false;
    }
